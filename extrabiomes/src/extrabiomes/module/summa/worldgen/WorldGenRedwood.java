                ...
                for (int z1 = z - k1; z1 <= z + k1; z1++)
                    if (y1 >= 0 && y1 < 256) {
                        ...
                        if (Block.blocksList[id] != null
                                && Block.blocksList[id].isLeaves(world, x1, y1, z1)) return false;
                    } else
                        return false;
        }
            ...
            for (int x1 = x - l1; x1 <= x + l1; x1++) {
                ...
                for (int z1 = z - l1; z1 <= z + l1; z1++) {
                            ...
                            && (block == null || block.canBeReplacedByLeaves(world, x1, y1, z1)))
                    {
                        ...
                        setBlockAndMetadata(world, x1 - 1, y1, z1, TreeBlock.LEAVES.getID(),
                                TreeBlock.LEAVES.getMetadata());
                        setBlockAndMetadata(world, x1, y1, z1 - 1, TreeBlock.LEAVES.getID(),
                                TreeBlock.LEAVES.getMetadata());
                        setBlockAndMetadata(world, x1 - 1, y1, z1 - 1, TreeBlock.LEAVES.getID(),
                                TreeBlock.LEAVES.getMetadata());
                    }
                }
            }
