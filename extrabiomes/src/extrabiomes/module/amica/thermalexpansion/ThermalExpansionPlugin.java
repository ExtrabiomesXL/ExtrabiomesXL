...
public class ThermalExpansionPlugin {
	...
	private Optional<ThermalExpansionAPI>	api			= Optional
																.absent();

	private void addSawmillRecipes() {
		if (!api.get().getSawmill().isPresent()) return;

		ItemStack sawdust;
		try {
			final Class cls = Class
					.forName("thermalexpansion.ThermalExpansionCore");
			final Field fld = cls.getField("sawdust");
			sawdust = (ItemStack) fld.get(null);
		} catch (final Exception e) {
			e.printStackTrace();
			sawdust = null;
		}

		final SawmillManager sawmill = api.get().getSawmill().get();
		...
		if (Stuff.log.isPresent() && Stuff.planks.isPresent()) {
			...
			ItemStack primaryOutput = new ItemStack(Stuff.planks.get(),
					6, BlockCustomWood.BlockType.ACACIA.metadata());
			sawmill.addRecipe(80, input, primaryOutput, sawdust);

			input = new ItemStack(Stuff.log.get(), 1,
	...
	public void preInit(PluginEvent.Pre event) {
		if (!Extrabiomes.proxy.isModLoaded("ThermalExpansion|Factory")
				|| !Extrabiomes.proxy
						.isModLoaded("ThermalExpansion|Core")) return;
