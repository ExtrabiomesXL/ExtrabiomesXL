...
public class VanillaFloraGenerator implements IWorldGenerator {
	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator, IChunkProvider chunkProvider) {
		...
		final boolean biomeIsExtremeJungle = BiomeManager.extremejungle.isPresent() && biome == BiomeManager.extremejungle.get();

		if (biomeIsExtremeJungle) {
			generateVines(random, chunkX, chunkZ, world);
		}

		final boolean biomeIsMiniJungle = BiomeManager.minijungle .isPresent() && biome == BiomeManager.minijungle.get();
		final boolean biomeIsTemperateRainforest = BiomeManager.temperaterainforest.isPresent() && biome == BiomeManager.temperaterainforest.get();
