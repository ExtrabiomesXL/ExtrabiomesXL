...
public class RenderMiniLog implements ISimpleBlockRenderingHandler {
	...
	public void renderInventoryBlock(Block block, int metadata, int modelID, RenderBlocks renderer) {
		...
		renderer.renderFaceZNeg(block, 0.0D, 0.0D, 0.1875D, block.getIcon(2, metadata));
		var4.draw();

		var4.startDrawingQuads();
		var4.setNormal(0.0F, 0.0F, 1.0F);
		renderer.renderFaceZPos(block, 0.0D, 0.0D, -0.1875D, block.getIcon(3, metadata));
		var4.draw();

		var4.startDrawingQuads();
		var4.setNormal(-1.0F, 0.0F, 0.0F);
		renderer.renderFaceXNeg(block, 0.1875D, 0.0D, 0.0D, block.getIcon(4, metadata));
		var4.draw();

		var4.startDrawingQuads();
		var4.setNormal(1.0F, 0.0F, 0.0F);
		renderer.renderFaceXPos(block, -0.1875D, 0.0D, 0.0D, block.getIcon(5, metadata));
		var4.draw();
	...
	public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer) {
        ...
        switch(orientation) {
	        case 4:
                ...
                if(((BlockMiniLog)Blocks.BLOCK_LOG_SAKURA_GROVE.get()).canConnect(world, x, y, z+1)) renderNorthNub(x, y, z, block, renderer);
                if(((BlockMiniLog)Blocks.BLOCK_LOG_SAKURA_GROVE.get()).canConnect(world, x, y, z-1)) renderSouthNub(x, y, z, block, renderer); 
                
                if(((BlockMiniLog)Blocks.BLOCK_LOG_SAKURA_GROVE.get()).canConnect(world, x, y+1, z)) renderUpNub(x, y, z, block, renderer); 
                if(((BlockMiniLog)Blocks.BLOCK_LOG_SAKURA_GROVE.get()).canConnect(world, x, y-1, z)) renderDownNub(x, y, z, block, renderer); 
                
	        	break;
	        case 8:
        		renderNorthSouthCenter(x, y, z, block, renderer);
                
                if(((BlockMiniLog)Blocks.BLOCK_LOG_SAKURA_GROVE.get()).canConnect(world, x-1, y, z)) renderEastNub(x, y, z, block, renderer); 
                if(((BlockMiniLog)Blocks.BLOCK_LOG_SAKURA_GROVE.get()).canConnect(world, x+1, y, z)) renderWestNub(x, y, z, block, renderer); 
