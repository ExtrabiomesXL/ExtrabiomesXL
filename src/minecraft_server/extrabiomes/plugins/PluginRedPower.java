...
public enum PluginRedPower implements IPlugin {
	...
	public void inject() {
		...
		config.load();
		Property plantsIDProperty = config.getOrCreateIntProperty(
				"redpower.plants.id", CATEGORY_PLUGIN, idPlants);
		...
		IBiomeDecoration plantsJungleForest = idPlants != 0 ? new BiomeDecorationPlants()
				: null;
		IBiomeDecoration plantsPlains = idPlants != 0 ? new BiomeDecorationPlants(
				4) : null;
