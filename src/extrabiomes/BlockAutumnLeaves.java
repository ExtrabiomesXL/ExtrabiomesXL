...
class BlockAutumnLeaves extends BlockLeavesBase implements MultiBlock,
		IShearable, ITextureProvider {

	private static final String BLOCK_NAME = "extrabiomes.autumnleaves";
	...
	private static final int APPLE_DROP_RARITY = 200;
	private static final int RANGE_WOOD_PREVENTS_DECAY = 4;

	private static Map<AutumnLeafType, MetaBlock> saplingDrops = null;
	...
	private static void loadCachedBlocks() {
		...
		saplingDrops = new EnumMap<AutumnLeafType, MetaBlock>(
				AutumnLeafType.class);

		for (AutumnLeafType i : AutumnLeafType.values())
	...
	public void addCreativeItems(final ArrayList itemList) {
		for (AutumnLeafType lt : AutumnLeafType.values())
	...
	protected int damageDropped(final int metadata) {
		...
		return saplingDrops.get(
				AutumnLeafType.fromMetadata(unmarkedMetadata(metadata)))
				.metadata();
	}
	...
	public void dropBlockAsItemWithChance(final World world, final int x,
		...
		if (world.rand.nextInt(APPLE_DROP_RARITY) == 0)
