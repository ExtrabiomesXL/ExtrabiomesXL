...
public class WorldGenTreesRed extends WorldGenerator
{
    ...
    public WorldGenTreesRed(boolean par1)
    {
        this(par1, 4, 0, 0, false);
    }

    public WorldGenTreesRed(boolean par1, int par2, int par3, int par4, boolean par5)
    {
        super(par1);
        field_48202_a = par2;
        field_48201_c = par3;
            ...
            for (int l = par3 - byte0; l <= par3 + byte0 && flag; l++)
            {
                for (int j1 = par5 - byte0; j1 <= par5 + byte0 && flag; j1++)
                {
                    if (j >= 0 && j < 256)
                    {
                        ...
                        if (j2 != 0 && j2 != mod_ExtraBiomesXL.leavesRed.blockID && j2 != Block.grass.blockID && j2 != Block.dirt.blockID && j2 != Block.wood.blockID)
                        {
                            flag = false;
                        }
                    }
        ...
        for (int k1 = (par4 - byte1) + i; k1 <= par4 + i; k1++)
        {
            ...
            for (int l3 = par3 - j3; l3 <= par3 + j3; l3++)
            {
                ...
                for (int l4 = par5 - j3; l4 <= par5 + j3; l4++)
                {
                    ...
                    {
                        setBlockAndMetadata(par1World, l3, k1, l4, mod_ExtraBiomesXL.leavesRed.blockID, field_48199_d);
                    }
                }
            }
        }
