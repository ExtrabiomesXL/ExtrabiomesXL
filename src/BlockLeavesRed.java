...
public class BlockLeavesRed extends BlockLeavesBase implements ITextureProvider
{
    private int baseIndexInPNG;
    int adjacentTreeBlocks[];

    protected BlockLeavesRed(int i, int j)
    {
            ...
            for (int j1 = -l; j1 <= l; j1++)
            {
                for (int k1 = -l; k1 <= l; k1++)
                {
                    for (int l1 = -l; l1 <= l; l1++)
                    {
                        ...
                        if (i2 == mod_ExtraBiomesXL.leavesRed.blockID)
                        {
                ...
                for (int l1 = -byte0; l1 <= byte0; l1++)
                {
                    for (int k2 = -byte0; k2 <= byte0; k2++)
                    {
                        for (int i3 = -byte0; i3 <= byte0; i3++)
                        {
                            ...
                            if (k3 == mod_ExtraBiomesXL.leavesRed.blockID)
                            {
    ...
    public void harvestBlock(World world, EntityPlayer entityplayer, int i, int j, int k, int l)
    {
        if (!world.isRemote && entityplayer.getCurrentEquippedItem() != null && entityplayer.getCurrentEquippedItem().itemID == Item.shears.shiftedIndex)
        {
            ...
            dropBlockAsItem_do(world, i, j, k, new ItemStack(mod_ExtraBiomesXL.leavesRed.blockID, 1, l & 3));
        }
        else
        {
